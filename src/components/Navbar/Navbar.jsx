...
const Navbar = () => {
  ...
  return (
    <header className={`bg-white-300 fixed top-0 right-0 left-0 z-50 ${isScrolled ? 'drop-shadow-[0_4px_26px_rgba(0,0,0,.8)]' : ""} `}>
      <nav className='max-w-[1100px] mx-auto px-10 md:h-[14vh] h-[12vh] flex justify-between items-center'>
        ...
        {/* Mobile Menu */}
        <ul className={`flex flex-col gap-y-12 bg-orange-500/15 backdrop-blur-xl shadow-xl  rounded-xl p-10 items-center md:hidden absolute top-20 -left-full transform -translate-x-1/2 transition-all duration-300 ${showMenu ? 'left-1/2' : ''}`}>       
          <li>
            <a href="#" className='font-semibold tracking-wider text-orange-800'>Home</a>
          </li>
          <li>
